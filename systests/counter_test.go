...
func (s *CounterTestSuite) TestShouldBeAbleToAddCounter() {
	...
	availableCounterHandlerClientA := testdata.NewMockAvailableCounterHandler(s.T())
	...
	availableCounterHandlerClientB := testdata.NewMockAvailableCounterHandler(s.T())
...
func (s *CounterTestSuite) TestShouldBeAbleToAddReadableCounterWithinHandler() {
	...
	s.Assert().Equal(counterLabel, readableCounter.Label())
}

func (s *CounterTestSuite) TestShouldBeAbleToAddReadableCounterWithinHandlerWithAddCounterByLabel() {
	ch, handler := s.createReadableCounterHandler()
	s.clientB.AddAvailableCounterHandler(handler)

	regId, err := s.clientA.AddCounterByLabel(counterTypeId, counterLabel)
	s.Require().NoError(err)
	var counter *aeron.Counter
	for counter == nil && err == nil {
		counter, err = s.clientA.FindCounter(regId)
...
func (s *CounterTestSuite) TestShouldBeAbleToAddReadableCounterAndGetCounterReads() {
	ch, handler := s.createReadableCounterHandler()
	s.clientB.AddAvailableCounterHandler(handler)
	...
	s.Assert().Equal(counter.Counter().Get(), readableCounter.Get())
